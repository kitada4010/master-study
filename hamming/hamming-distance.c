...
int main(void){
  ...
  FILE *hamming_pair; //ハミング距離3のペアを出力するtexファイルのファイルポインタ
  int j=0, k;
  unsigned i;
  ...
  if((hamming_pair = fopen("hamming_pair.tex", "w")) == NULL){
  ...
  for(j=0; j<16; j++){  //ハミング距離の導出
	...
	if(bit_count == 3){
	  if(patern_count%8 == 0 && patern_count != 0)
	    fprintf(hamming_pair, "\\\\");
	  patern_count++;
	  fprintf(hamming_pair, "\\left\\{x_{%d}, x_{%d}\\right\\},", j+1,k+1);  //ハミング距離が3のペアを出力
	}
